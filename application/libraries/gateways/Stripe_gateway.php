    ...
    {   
        ...
        if (isset($data['pay_with_card'])) {
            ...
            $charge['source'] = !$sourceFound
                ? $stripeCustomer->sources->create(['source' => $stripeToken])
                : $sourceFound;
        ...
        if($data['invoicedata']->recurring != 0)
        {   

            $recurringType = "month";
            if(!empty($data['invoicedata']->recurring_type))
            {
                $recurringType = $data['invoicedata']->recurring_type;
            }
        
            $id_hash = uniqid();
        $plan =  array("amount" =>  strcasecmp($data['currency'], 'JPY') == 0 ? intval($data['amount']) : $data['amount'] * 100,"interval" =>$recurringType,
                "interval_count"=>round($data['invoicedata']->recurring),
            "product" => array("name" => $data['invoicedata']->prefix."".$id_hash),'nickname' => $data['invoicedata']->prefix."".$id_hash,"currency" => $data['currency'],"id" => $data['invoicedata']->prefix."".$id_hash);

            $plan_response = $this->ci->stripe_core->create_plan($plan);

            if($data['invoicedata']->autopay == 1)
            {
                $billing = "charge_automatically";
                $days = null;
            }
            elseif($data['invoicedata']->autopay == 0)
            {
                $billing = "send_invoice";
                $days = round($data['invoicedata']->recurring*30);
                if($data['invoicedata']->recurring_type == 'day')
                {
                    $days = round($data['invoicedata']->recurring*1);
                }
                else if($data['invoicedata']->recurring_type == 'week')
                {
                    $days = round($data['invoicedata']->recurring*7);
                }
                else if($data['invoicedata']->recurring_type == 'year')
                {
                    $days = round($data['invoicedata']->recurring*365);
                }
                else if(empty($data['invoicedata']->recurring_type))
                {
                    $days = round($data['invoicedata']->recurring*30);
                }  
            ...
            $subscription = array("customer"=>$stripeCustomerId,"items" => array(array("plan"=>$data['invoicedata']->prefix."".$id_hash)),"billing"=>$billing,"days_until_due"=>$days);
            $result = $this->ci->stripe_core->create_subscription($subscription);
