...
class Cron_model extends CRM_Model
{
    ...
    public function recurring_invoices()
    {
        ...
        if ($hour_now != $invoice_hour_auto_operations && $this->manually === true) {
            return;
        }
        ...
        foreach ($invoices as $invoice) {
            ...
            if (date('Y-m-d') >= $re_create_at) 
            {
                ...
                foreach ($_invoice->items as $item) {
                    ...
                    if (strpos($item['long_description'], '{service_date}') !== false) {

                         $desp  = str_replace("{service_date}",$monthdates,$item['long_description']);
                    }
                    else
                    {
                        $desp = $item['long_description'];
                    }

        
                    $new_invoice_data['newitems'][$key]['description']      = $item['description'];
                    $new_invoice_data['newitems'][$key]['long_description'] = clear_textarea_breaks($desp);
                ...
                if(!empty($clientData->stripe_id)){

                    $subs = $this->stripe_core->get_subscriptions($clientData->stripe_id);
                    $subscriptionId = $subs->data[0]->id;

                    $aPk = array("subscription"=>$subscriptionId);
                    $allInvoices = $this->stripe_core->allinvoices($aPk);
                ...
                if ($id) {
                    ...
                    if ($new_recurring_invoice_action == 'generate_and_send') {
                        ...
                        $this->invoices_model->send_invoice_to_client($id, 'Autopay_invoice_send_to_customer', true);
    ...
    public function updateInvoicesStatus()
    {
         $this->load->model('invoices_model');
          $this->load->library('stripe_core');
        ...
        foreach ($clients as $client) {
            ...
            if(isset($subs->data[0])){
                ...
                foreach ($allInvoices['data'] as $k4 => $v2) {
                    ...
                    $invDate = date('Y-m-d', $v2->date);
                    $status = $v2->status;
                    $strpData = array("date"=>$invDate,"status"=>$status);
                    array_push($stripeInvoices, $strpData);       
                }
                foreach ($stripeInvoices as $sk => $sv) {
                
                if ($sv['status'] == "paid") {
                    foreach ($invoicesInCRM as $ik => $iv) {
                        
                        if ($iv['date'] == $sv['date'] ) {
                            
                            $updateWhere["id"] = $iv['id'];
