...
class Invoices_model extends CRM_Model
{
    ...
    function get_recursive_invoice($where = [])
    {
        $this->db->select("*");
        $this->db->from(db_prefix(). 'invoices');
        $this->db->where($where);
    ...
    public function send_invoice_overdue_notice($id)
    {
        ...
        $data['notice'] = "Our records indicate that we have not recived your payment and your account is past due";
        ...
        $attach = format_invoice_number($id).".pdf";

        file_put_contents($attach, $file);
    ...
    public function send_invoice_to_client($id, $template_name = '', $attachpdf = true, $cc = '', $manually = false)
    {
        ...
        if (is_array($sent_to) && count($sent_to) > 0) {
            ...
            // Render the HTML as PDF
                ...
                $attach =  $this->pdf->output();
            // Get the generated PDF file contents
                $name = format_invoice_number($id).".pdf";

                file_put_contents($name, $attach);
            }
    ...
    function client_recurring_invoice($where)
    {
       $this->db->select("*");
       $this->db->from(db_prefix() . 'invoices');
       $this->db->where($where);
    ...
    function get_early_expire_invioce($order_by,$odr_method)
    {
       $this->db->select("*");
       $this->db->from(db_prefix() . 'invoices');
    ...
    function updateInvoice($where,$data)
    {
        $this->db->where($where);
        $res = $this->db->update(db_prefix() . 'invoices', $data);
        return $res;
    }
    ...
    function getInvoices($clientID)
    {
        ...
        $this->db->from(db_prefix() . 'invoices');
        $this->db->where('(recurring != 0 OR is_recurring_from IS NOT null)');
    ...
    function getInvoicePayDate($invoice)
    {
        $this->db->select('*');
